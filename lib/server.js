    ...
    function initPublicRepoCache() {
        ...
        if(_config.repositoryCache.svn && _config.repositoryCache.svn.enabled) {
            repoCacheOptions.svn = {
                ...
                publicAccessURL: _config.repositoryCache.svn.publicAccessURL || null,
...
function _initService() {
    ...
    if(_config.host) {
        app.listen(_config.port, _config.host, function() {
            logger.log('Bower server started on ' + _config.host + ':' + _config.port);
        });
    } else {
