...
function _initService() {
    ...
    app.get('/packages/search/:name', function(req, res) {
        ...
        function filterPublicPackages(pkg){
            return _config.publicPackagesWhitelist.indexOf(pkg.name) > -1;
        }

        if(!_config.disablePublic) {
            Array.prototype.push.apply(packages, !_config.publicPackagesWhitelist
                ? _publicPackageStore.searchPackage(searchName)
                : _publicPackageStore.searchPackage(searchName).filter(filterPublicPackages));
        }
    ...
    app.get('/packages/:name', function(req, res) {
        ...
        if(privatePackage) {
            ...
            (!_config.publicPackagesWhitelist || _config.publicPackagesWhitelist.indexOf(packageName) > -1)) {
            handlePublicPackage();
        }
        else {
