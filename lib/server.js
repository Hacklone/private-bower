...
function _initService() {
    logger.log('server._initService()');
    app.use(express.bodyParser());
    app.use(express.static(path.join(__dirname, '../site')));

    app.post('/registerPackage', function(req, res) {
        logger.log('/registerPackage');
    ...
    app.get('/packages/search/:name', function(req, res) {
        logger.log('/packages/search/:name');
    ...
    app.get('/packages/:name', function(req, res) {
        logger.log('/packages/:name');
        var name = req.params.name;
        var privatePackage = _packageStore.getPackage(name);

        if(privatePackage) {
            logger.log('req.params.name: {0}'.format(req.params.name));
            logger.log('server:privatePackage("{ name:{0}, url:{1}, hits:{2} }")'.format(privatePackage.name, privatePackage.repo, privatePackage.hits));
            logger.log('res.send(privatePackage)@1');
            res.send({
                ...
                if(_repoCacheEnabled) {
                    _publicRepoCache.cacheRepo(req.params.name, publicPackage.url)
                        .then(function(pack) {
                            ...
                            logger.log('server:privatePackage("{ name:{0}, url:{1}, hits:{2} }")'.format(privatePackage.name, privatePackage.repo, privatePackage.hits));
                            logger.log('res.send(privatePackage)@2');
                            res.send(privatePackage);
                        })
                        .fail(function() {
                            logger.log('server:publicPackage("{ name:{0}, url:{1}, hits:{2} }")'.format(publicPackage.name, publicPackage.url, publicPackage.hits));
                            logger.log('res.send(publicPackage)@1');
                            res.send(publicPackage);
                        });
                }
                else {
                    logger.log('server:publicPackage("{ name:{0}, url:{1}, hits:{2} }")'.format(publicPackage.name, publicPackage.url, publicPackage.hits));
                    logger.log('res.send(publicPackage)@2');
                    res.send(publicPackage);
                }
            }
        }
