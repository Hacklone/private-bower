...
module.exports = function PublicRepoCache(options) {
    ...
    function _cacheRepo(repoName, repoUrl) {
        logger.log('_cacheRepo(repoName:{0}, repoUrl{1})'.format(repoName, repoUrl));
    ...
    function _createDirectory(dir) {
        logger.log('_createDirectory(dir:{0})'.format(dir));
        var deferred = q.defer();
    ...
    function _cloneGitRepo(repoUrl, packageDirectory, repoName) {
        logger.log('_cloneGitRepo(repoUrl:{0), packageDirectory:{1}, repoName{2}'.format(repoUrl, packageDirectory, repoName));
        var deferred = q.defer();
