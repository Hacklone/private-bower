...
module.exports = function PublicRepoCache(options) {
    _init();
    function _init() {
        logger.log('public-repo-cache._init()');
    ...
    function _cacheRepo(repoName, repoUrl) {
        logger.log('public-repo-cache._cacheRepo(repoName:{0}, repoUrl{1})'.format(repoName, repoUrl));
    ...
    function _createDirectory(dir) {
        logger.log('public-repo-cache._createDirectory(dir:{0})'.format(dir));
        var deferred = q.defer();
    ...
    function _startGitDaemon() {
        logger.log('public-repo-cache._startGitDaemon()');
        var deferred = q.defer();
        ...
        logger.log('$ {0}'.format(gitCommand));
    ...
    function _cloneGitRepo(repoUrl, packageDirectory, repoName) {
        logger.log('public-repo-cache._cloneGitRepo(repoUrl:{0), packageDirectory:{1}, repoName{2})'.format(repoUrl, packageDirectory, repoName));
        var deferred = q.defer();
