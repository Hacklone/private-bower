...
module.exports = function SvnRepoCache(options) {
    ...
    function _initRepository() {
      ...
      exec("svnlook info .", function(error, stdout, stderr) {
        if(error) {
          //need to initialize repository
          exec("svnadmin create .", function(error, stdout, stderr) {
            if(error) {
        ...
        tmp.dir(
          ...
          function(err, path, cleanup) {
            ...
            exec(svnCommand, function(error, stdout, stderr) {
              ...
              exec("svn import . file://{0} -m IMPORT > /dev/null".format(packageDirectory), function(error, stdout, stderr) {
                delete cloningPromises[packageDirectory];
                cleanup()
                if(error) {
                  deferred.reject(stderr);
