...
module.exports = function PublicPackageStore(config) {
    ...
    function _getPackage(packageName) {
        logger.log('public-package-store._getPackage(packageName:{0})'.format(packageName));
        return _packages[packageName];
    }

    function _loadPublicPackages() {
        logger.log('public-package-store._loadPublicPackages()');
...
//            logger.log('public-package-store.processData(data:{0})'.format(data));
            logger.log('public-package-store.processData(data:[ { "name": name, "url" : url, "hits" : hits }, ])');
        ...
        function createClient() {
            logger.log('public-package-store.createClient()');
            var clientOptions;
    ...
    function _searchPackage(name) {
        logger.log('public-package-store._searchPackage(name:{0})'.format(name));
